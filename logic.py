...
Resultado del escaneo:
  ...
  - "fuera_horario" → gris, no hay comida activa en este momento
"""
...
def procesar_scan(raw_input: str) -> dict:
    ...
    comida = utils.comida_activa_ahora(comidas)
    if comida is None:
        proxima = utils.comida_actual(comidas)
        result["estado"] = "fuera_horario"
        result["comida"] = dict(proxima) if proxima else None
        if proxima:
            result["mensaje"] = (
                f"Fuera de horario de atención. Próxima comida: "
                f"{proxima['nombre']} ({proxima['hora_inicio']}–{proxima['hora_fin']})"
            )
        else:
            result["mensaje"] = "Fuera de horario de atención"
        return result
    ...
    # 8b. Advertencia de cupo (no bloquea el ingreso — la comida ya fue servida)
    cap_info = get_capacidad_info()
    result["cupo_excedido"]     = cap_info["disponibles"] <= 0
    result["cupo_disponibles"]  = cap_info["disponibles"]
    result["cupo_total"]        = cap_info["total_cupos"]
...
def detectar_ausencias_previas(run: str, comida_actual_id: int,
    ...
    for i in range(idx_actual - 1, -1, -1):
        ...
        if not db.ya_registrado(run, cid, hoy_str):
            # Si nadie fue registrado en esa comida → PAE no operó ese turno → sin strike
            if db.count_registros_comida(hoy_str, cid) == 0:
                break
    ...
    if idx_actual == 0:
        ...
        if total_registros_ayer == 0:
            return faltadas   # día sin servicio, no penalizar a nadie
        ...
        if not ids_asistio_ayer:
            ...
            # Solo strike si efectivamente hubo servicio en esa comida ayer
            if (ultima_cid not in ids_asistio_ayer
                    and db.count_registros_comida(ayer, ultima_cid) > 0):
                faltadas.append({
